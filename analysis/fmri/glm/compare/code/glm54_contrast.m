...
contrast = {'BOCPD-DV(+)','BOCPD-DV(-)','DBM-DV(+)','DBM-DV(-)',...
    'HEU-DV(+)','HEU-DV(-)','BOCPD-DV>other','DBM-DV>other','HEU-DV>other'};
...
for iSub = 1:length(subjects)
    ...
    matlabbatch{iJob}.spm.stats.con.spmmat = ...
        {[rp 'analysis/fmri/glm/glm54/result/1stlevel/' subjects{iSub} '/SPM.mat']};
    ...
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.weights = [0 0 0 1 -0.5 -0.5];
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.sessrep = 'none';
    
    iCont = iCont + 1;
    
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.name = contrast{8};
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.weights = [0 0 0 -0.5 1 -0.5];
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.sessrep = 'none';
    
    iCont = iCont + 1;
    
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.name = contrast{9};
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.weights = [0 0 0 -0.5 -0.5 1];
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.sessrep = 'none';
...
if ~exist([rp 'analysis/fmri/glm/glm54/result/2ndlevel'], 'dir')
    mkdir([rp 'analysis/fmri/glm/glm54/result/'], '2ndlevel');
else
    rmdir([rp 'analysis/fmri/glm/glm54/result/2ndlevel'],'s');
    mkdir([rp 'analysis/fmri/glm/glm54/result/'], '2ndlevel');
end

for iCon = 1:length(contrast)
    
    if ~exist([rp 'analysis/fmri/glm/glm54/result/2ndlevel/' ...
            contrast{iCon}], 'dir')
        mkdir([rp 'analysis/fmri/glm/glm54/result/2ndlevel/' ...
            contrast{iCon}]);
    end
    ...
    matlabbatch{iJob}.spm.stats.factorial_design.dir = ...
        {[rp 'analysis/fmri/glm/glm54/result/2ndlevel/' contrast{iCon}]};
    
    for iSub = 1:numel(subjects)
        if iCon <= 9
            matlabbatch{iJob}.spm.stats.factorial_design.des.t1.scans{iSub} = ...
                [rp 'analysis/fmri/glm/glm54/result/1stlevel/' ...
    ...
    matlabbatch{iJob}.spm.stats.fmri_est.spmmat = ...
        {[rp 'analysis/fmri/glm/glm54/result/2ndlevel/' contrast{iCon} '/SPM.mat']};
