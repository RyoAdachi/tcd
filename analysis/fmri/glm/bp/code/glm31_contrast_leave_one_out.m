...
function glm31_contrast_leave_one_out(subjectName)
...
contrast = {'BP(main-control)','BP(control-main)'};

% Create the directories for SPM.mat to be saved
if ~exist([rp 'analysis/fmri/glm/glm31/result/2ndlevel_leave_one_out'], 'dir')
    mkdir([rp 'analysis/fmri/glm/glm31/result/'], '2ndlevel_leave_one_out');
end
...
if ~exist([rp 'analysis/fmri/glm/glm31/result/2ndlevel_leave_one_out/' subjectName], 'dir')
    mkdir([rp 'analysis/fmri/glm/glm31/result/2ndlevel_leave_one_out'], subjectName);
else
    rmdir([rp 'analysis/fmri/glm/glm31/result/2ndlevel_leave_one_out/' subjectName],'s');
    mkdir([rp 'analysis/fmri/glm/glm31/result/2ndlevel_leave_one_out'], subjectName);
end

for iCon = 1:length(contrast)
    
    if ~exist([rp 'analysis/fmri/glm/glm31/result/2ndlevel_leave_one_out/' ...
    ...
    matlabbatch{iJob}.spm.stats.factorial_design.dir = ...
        {[rp 'analysis/fmri/glm/glm31/result/2ndlevel_leave_one_out/' ...
        subjectName '/' contrast{iCon}]};
    
    for j = 1:numel(subject_out)
        if iCon <= 9
            matlabbatch{iJob}.spm.stats.factorial_design.des.t1.scans{j} = ...
                [rp 'analysis/fmri/glm/glm31/result/1stlevel/' ...
