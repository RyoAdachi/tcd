...
for iSub = 1:length(subjects)
    ...
    matlabbatch{iJob}.spm.stats.con.spmmat = ...
        {[rp 'analysis/fmri/glm/glm31/result/1stlevel/' subjects{iSub} '/SPM.mat']};
    
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.name = contrast{1};
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.weights = [0 1 -1];
...
if ~exist([rp 'analysis/fmri/glm/glm31/result/2ndlevel'], 'dir')
    mkdir([rp 'analysis/fmri/glm/glm31/result/'], '2ndlevel');
else
    rmdir([rp 'analysis/fmri/glm/glm31/result/2ndlevel'],'s');
    mkdir([rp 'analysis/fmri/glm/glm31/result/'], '2ndlevel');
end

for iCon = 1:length(contrast)
    
    if ~exist([rp 'analysis/fmri/glm/glm31/result/2ndlevel/' ...
            contrast{iCon}], 'dir')
        mkdir([rp 'analysis/fmri/glm/glm31/result/2ndlevel/' ...
            contrast{iCon}]);
    end
    ...
    matlabbatch{iJob}.spm.stats.factorial_design.dir = ...
        {[rp 'analysis/fmri/glm/glm31/result/2ndlevel/' contrast{iCon}]};
    ...
    matlabbatch{iJob}.spm.stats.fmri_est.spmmat = ...
        {[rp 'analysis/fmri/glm/glm31/result/2ndlevel/' contrast{iCon} '/SPM.mat']};
