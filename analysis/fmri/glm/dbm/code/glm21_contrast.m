%% Contrast creator for glm21
% glm21 TESTS THE REGIONS CORRELATING WITH
% (1) IMAGE PRESENTATION
% (2) BUTTON PRESS
% (3) every 250ms
% (4) DBM, currentmeanPostEps
% (5) DBM, previousmeanPostEps
% (6) DBM, varPostEps
% (7) DBM, diffMean
% (8) Decision period (control)

function glm21_contrast()
...
contrast = {'currentmeanPost(+)','currentmeanPost(-)','prevmeanPost(+)','prevmeanPost(-)',...
    'varPost(+)','varPost(-)','DV(+)','DV(-)','every250ms(+)','every250ms(-)'};
...
for iSub = 1:length(subjects)
    ...
    matlabbatch{iJob}.spm.stats.con.spmmat = ...
        {[rp 'analysis/fmri/glm/glm21/result/1stlevel/' subjects{iSub} '/SPM.mat']};
    ...
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.weights = [0 0 0 0 0 1];
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.sessrep = 'none';
    ...
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.name = contrast{7};
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.weights = [0 0 0 0 0 0 1];
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.sessrep = 'none';
    
    iCont = iCont + 1;
    
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.name = contrast{8};
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.weights = [0 0 0 0 0 0 -1];
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.sessrep = 'none';
    
    iCont = iCont + 1;
    
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.name = contrast{9};
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.weights = [0 0 1];
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.sessrep = 'none';
    
    iCont = iCont + 1;
    
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.name = contrast{10};
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.weights = [0 0 -1];
    matlabbatch{iJob}.spm.stats.con.consess{iCont}.tcon.sessrep = 'none';
...
if ~exist([rp 'analysis/fmri/glm/glm21/result/2ndlevel'], 'dir')
    mkdir([rp 'analysis/fmri/glm/glm21/result/'], '2ndlevel');
else
    rmdir([rp 'analysis/fmri/glm/glm21/result/2ndlevel'],'s');
    mkdir([rp 'analysis/fmri/glm/glm21/result/'], '2ndlevel');
end

for iCon = 1:length(contrast)
    
    if ~exist([rp 'analysis/fmri/glm/glm21/result/2ndlevel/' ...
            contrast{iCon}], 'dir')
        mkdir([rp 'analysis/fmri/glm/glm21/result/2ndlevel/' ...
            contrast{iCon}]);
    end
    ...
    matlabbatch{iJob}.spm.stats.factorial_design.dir = ...
        {[rp 'analysis/fmri/glm/glm21/result/2ndlevel/' contrast{iCon}]};
    
    for iSub = 1:numel(subjects)
        if iCon <= 9
            matlabbatch{iJob}.spm.stats.factorial_design.des.t1.scans{iSub} = ...
                [rp 'analysis/fmri/glm/glm21/result/1stlevel/' ...
    ...
    matlabbatch{iJob}.spm.stats.fmri_est.spmmat = ...
        {[rp 'analysis/fmri/glm/glm21/result/2ndlevel/' contrast{iCon} '/SPM.mat']};
